...
class _SigninScreenState extends State<SigninScreen> {
  ...
  Future<String?> _login() async {
    ...
    try {
      // Authenticate FIRST so wrong credentials show proper errors
      ...
      // After successful auth, check maintenance mode
      final maintenanceService = MaintenanceService.instance;
      final isMaintenance = await maintenanceService.forceCheck();
      ...
      if (isMaintenance) {
        ...
        final isAdmin = adminSnapshot.exists && adminSnapshot.value == 'admin';

        if (!isAdmin) {
          // Student/Teacher: send a maintenance notification and show dialog
          await _addMaintenanceNotification(user.uid);

          if (mounted) {
            await maintenanceService.showMaintenanceDialog(context);
          }
          // Still return the UID — they can log in but will see the banner
        }
      }
  ...
  /// Add a notification to the user about maintenance mode
  Future<void> _addMaintenanceNotification(String uid) async {
    try {
      final maintenanceService = MaintenanceService.instance;
      await FirebaseDatabase.instance
        ...
        'message':
            '${maintenanceService.maintenanceMessage} Scheduled: ${maintenanceService.maintenanceTimeFrame}. Some features may be temporarily unavailable.',
        'type': 'maintenance',
        'isRead': false,
        'createdAt': ServerValue.timestamp,
      });
    } catch (e) {
      debugPrint('Error adding maintenance notification: $e');
    }
  }
