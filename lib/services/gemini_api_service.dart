...
/// AI Service for EduVerse using Google Gemini API
///
/// Architecture:
/// - Uses Gemini API directly for both web and mobile
...
''',
  ...
  static String? _getFallbackResponse(String prompt) {
    ...
    if (lowerPrompt.contains('world war') || lowerPrompt.contains('ww2') || lowerPrompt.contains('wwii')) {
    ...
    if (lowerPrompt.contains('biology') || lowerPrompt.contains('project idea')) {
      return _fallbackResponses['suggest project ideas for biology'];
    }
    if (lowerPrompt.contains('photosynthesis') || lowerPrompt.contains('plant')) {
      return _fallbackResponses['what is photosynthesis'];
    }
    if (lowerPrompt.contains('newton') || lowerPrompt.contains('motion') || lowerPrompt.contains('force')) {
      return _fallbackResponses['explain newton\'s laws of motion'];
    }
    if (lowerPrompt.contains('machine learning') || lowerPrompt.contains('ai') || lowerPrompt.contains('artificial intelligence')) {
  ...
  // Gemini API URL - using gemini-2.0-flash for latest model
  ...
  Duration _getBackoffDelay() {
    ...
    final seconds = (_baseRequestInterval.inSeconds * (1 << _consecutiveRateLimits))
        .clamp(2, _maxBackoffSeconds);
    return Duration(seconds: seconds);
  }

  /// Send a message to the AI and get a response
  Future<String> sendMessage(String prompt, {String? systemPrompt, int retryCount = 0}) async {
    ...
    if (apiKey != null && apiKey!.isNotEmpty) {
      try {
        final response = await _callGeminiApi(prompt, systemPrompt: systemPrompt);
        ...
        if (errorStr.contains('rate_limit') || errorStr.contains('429')) {
          ...
          if (retryCount < 3) {
            ...
            if (kDebugMode) {
              print('Rate limited. Waiting ${waitTime.inSeconds}s before retry #${retryCount + 1}');
            }
            await Future.delayed(waitTime);
            return sendMessage(prompt, systemPrompt: systemPrompt, retryCount: retryCount + 1);
          }
        }
  ...
  Future<String> _callGeminiApi(String prompt, {String? systemPrompt}) async {
    ...
    final systemInstruction = systemPrompt ?? 
        'You are EduVerse AI, a helpful educational assistant. Provide clear, concise, and educational responses. Help students learn and understand concepts better.';
  ...
  /// Analyze an image using Gemini Vision API (for homework help feature)
  Future<String> analyzeImage(String base64Image, {String? prompt, int retryCount = 0}) async {
    // Rate limiting with exponential backoff
    if (_lastRequestTime != null) {
      final backoffDelay = _getBackoffDelay();
      final elapsed = DateTime.now().difference(_lastRequestTime!);
    ...
    // Try Gemini Vision API directly
    if (apiKey != null && apiKey!.isNotEmpty) {
      try {
        final response = await _callGeminiVisionApi(base64Image, prompt: prompt);
        _consecutiveRateLimits = 0; // Reset on success
        return response;
      } catch (e) {
        final errorStr = e.toString().toLowerCase();
        ...
        if (errorStr.contains('rate_limit') || errorStr.contains('429')) {
          ...
          if (retryCount < 3) {
            ...
            await Future.delayed(waitTime);
            return analyzeImage(base64Image, prompt: prompt, retryCount: retryCount + 1);
          }
        }
  ...
  /// Call Gemini Vision API directly for image analysis
  Future<String> _callGeminiVisionApi(String base64Image, {String? prompt}) async {
    if (apiKey == null || apiKey!.isEmpty) {
      throw Exception('API key not configured');
    }

    // Use gemini-2.0-flash for vision tasks (latest model with vision support)
    final url = Uri.parse(
    ...
    if (response.statusCode == 200) {
      ...
      if (candidates != null && candidates.isNotEmpty) {
        ...
        final parts = content['parts'] as List?;
        if (parts != null && parts.isNotEmpty) {
          return parts[0]['text'] ?? 'Could not analyze the image.';
        }
      }
      throw Exception('Invalid response format');
    } else if (response.statusCode == 429) {
...
final aiService = GeminiApiService(apiKey: _apiKey);